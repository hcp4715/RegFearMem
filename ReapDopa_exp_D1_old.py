...
expName = 'reapp_consolid_main'  # from the Builder filename that created this script
...
trialFilename = _thisDir + os.sep + u'data/%s_%s_%s' % (expInfo['participantID'],expInfo['date'],'_trial_d1.txt')
...
blockFilename = _thisDir + os.sep + u'data/%s_%s_%s' % (expInfo['participantID'],expInfo['date'],'_block_d1.txt')
...
def run_block(sessID,blockID,minblocktype,trialList,trialFile):
    ...
    showImage_c = visual.ImageStim(win, image=imName_c,pos=[0, -250]) # keep showing the cure under the lower part of the screeen
...
for ii in range(len(blocks)):
    ...
    for jj in range(len(curSess)):
        ...
        if SessID == twoShockSes and blockID == twoShockBlock:
            trials = [CSplus_1]*2+[CSplus_2]*2+[CSminus]*3
        else:
