    ...
    **Example 3**:
    ...
    def draw(self, log=True):
        ...
        if not self.mouseOnly:
            for key in self.io.devices.keyboard.getPresses(self.allKeys, clear=True):
                self.keyCache.append(key)
                print(self.keyCache)
            for key in self.io.devices.keyboard.getReleases(self.allKeys, clear=True):
        ...
        if len(self.keyCache) > 0:
            # force Psychopy to wait for a while
            core.wait(self.slidingInterval)
